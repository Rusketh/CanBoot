...
- [`display`](display.md) — pixel/rect/text painter (the image module sits on top of `hal_display_image`)
- [`fb`](fb.md) — `fb.flush()` to push the framebuffer to scanout on virtio-gpu paths
- [`fs`](fs.md) — `fs.read` to pull image bytes off a partition
