...
- **Up to ~1.5 sec lookahead.** Samples queued into the HAL ring stay
  ...
  Call `audio.stop()` then wait one pump cycle if you need a hard
  cutoff.
...
- [`fmt.sineWave16`](fmt.md) — synthesising test tones inline
- [`fs`](fs.md) — `fs.read` to pull audio bytes off a partition
