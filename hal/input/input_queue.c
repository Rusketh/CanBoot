...
int hal_input_getc(void) {
    ...
    while (hal_input_poll(&ev)) {
        if (ev.type == CANBOOT_EV_KEY_DOWN && ev.code >= 0x20 && ev.code < 0x80) {
            return (int)ev.code;
        }
        if (ev.type == CANBOOT_EV_KEY_DOWN &&
            (ev.code == CANBOOT_KEY_ENTER ||
             ev.code == CANBOOT_KEY_BACKSP ||
             ev.code == CANBOOT_KEY_TAB)) {
            switch (ev.code) {
