/*
 * Port-CF8/CFC PCI config space access + bus enumeration for x86.
 * Touches only standard PCI legacy ports; works on both BIOS and UEFI
 * paths after firmware hands off because PCI configuration mechanism 1
 * is part of the chipset, not the firmware.
 */
...
static uint32_t cf8_for(struct canboot_pci_addr a, uint8_t off) {
    return 0x80000000u
        | ((uint32_t)(a.bus & 0xFFu) << 16)
        | ((uint32_t)(a.dev & 0x1Fu) << 11)
        | ((uint32_t)(a.func & 0x07u) << 8)
        | ((uint32_t)(off  & 0xFCu));
}

uint32_t hal_pci_cfg_read32(struct canboot_pci_addr a, uint8_t off) {
    outl(CONFIG_ADDRESS, cf8_for(a, off));
    return inl(CONFIG_DATA);
}
uint16_t hal_pci_cfg_read16(struct canboot_pci_addr a, uint8_t off) {
    outl(CONFIG_ADDRESS, cf8_for(a, off));
    return (uint16_t)((inl(CONFIG_DATA) >> ((off & 2) * 8)) & 0xFFFFu);
}
uint8_t hal_pci_cfg_read8(struct canboot_pci_addr a, uint8_t off) {
    outl(CONFIG_ADDRESS, cf8_for(a, off));
    return (uint8_t)((inl(CONFIG_DATA) >> ((off & 3) * 8)) & 0xFFu);
}
...
void hal_pci_cfg_write16(struct canboot_pci_addr a, uint8_t off, uint16_t v) {
    ...
    uint32_t cur   = hal_pci_cfg_read32(a, off & 0xFCu);
    cur = (cur & ~mask) | (((uint32_t)v << shift) & mask);
    outl(CONFIG_ADDRESS, cf8_for(a, off & 0xFCu));
